...
// 20240206 PHR Added fiscal_year to group serach
// 20240305 PHR Added strtolower for better recognition.
// 20240416 LOE $kundeordre initialized
...
// 20240809 PHR Corrected error in $baseCurrency
// 20240815 PHR Translations in function kontoopslag & ansatopslag
		...
		if ($fejl == 0) {
			...
			while ($row = db_fetch_array($query)) {
				if (($row['posnr'] > 0) && (strlen(trim(($row['varenr']))) > 0)) {
					...
					$variant_id[$x] = (int) $row['variant_id'];
					if ($hurtigfakt == 'on')
						$leveres[$x] = $antal[$x];
					$lager[$x] = $row['lager'] * 1;
					if (!$lager[$x])
			...
			for ($x = 1; $x <= $linjeantal; $x++) {
				...
				if ($art != 'PO' && $serienr[$x]) {
					...
					if ($leveres[$x] < 0) {
						$sn_antal[$x] = 0;
						if ($art == 'DK')
							$qtxt = "select * from serienr where salgslinje_id = $kred_linje_id[$x]*-1";# 20121001 Rettet KO til DK 
						else
...
function linjeopdat($id, $gruppe, $linje_id, $beholdning, $vare_id, $antal, $pris, $nettopris, $rabat, $samlevare, $linje_nr, $posnr, $serienr, $kred_linje_id, $bogf_konto, $variant_id, $lager)
{
	...
	$query = db_select("select * from grupper where art='VG' and kodenr='$gruppe' and fiscal_year = '$regnaar'", __FILE__ . " linje " . __LINE__); #VG = Varegruppe
	...
	if ($box8 != 'on') { #box 8 angiver om vare(gruppen) er lagerført #20130408
		if ($bogf_konto)
			$box4 = $bogf_konto; # hvis funktionen kaldes med en bogføringskonto overruler denne box4
		...
		if ($r['id']) {
			...
			$qtxt = "insert into lagerstatus (vare_id,variant_id,beholdning,lager) values ('$vare_id','$variant_id','$tmp','$lager')";
		db_modify($qtxt, __FILE__ . " linje " . __LINE__);
		if ($variant_id) {
		...
		if ($box9 == 'on') { # #box 9 angiver om vare(gruppen) er underlagt batchkontrol
			if ($antal < 0)
				krediter($id, $levdate, $beholdning, $vare_id, $antal * -1, $pris, $linje_id, $serienr, $kred_linje_id, $lager, $variant_id);
			else
				batch_salg_lev($id, $levdate, $fakturadate, $beholdning, $vare_id, $antal, $pris, $nettopris, $linje_id, $linje_n, $posnr, $serienr, $lager, $variant_id);
		} else {
			if ($bogf_konto)
				$box4 = $bogf_konto;
			...
			if ($art == 'DK' || $antal < 0) {
				...
				$bk_id = 0;
				if ($kred_linje_id)
					$qtxt = "select * from batch_salg where linje_id = '$kred_linje_id' order by id desc"; #20170507
				else
					$qtxt = "select * from batch_salg where linje_id = '$linje_id' and batch_kob_id != 0  order by id desc";
				$q = db_select($qtxt, __FILE__ . " linje " . __LINE__);
				while ($r = db_fetch_array($q)) {
					$bs_antal = $r['antal'];
					$bs_id = $r['id'];
...
function bogfor_levering($id, $gruppe, $linje_id, $antal, $box1, $box2, $box3, $box4)
{
	...
	if ($beholdning < $antal)
		$antal = $beholdning; #lagerværdi flyttes til varekøb for den del af leveringen som ér på lager
...
function batch_salg_lev($id, $levdate, $fakturadate, $beholdning, $vare_id, $antal, $pris, $nettopris, $linje_id, $linje_nr, $posnr, $serienr, $lager, $variant_id)
{
	...
	if (!db_fetch_array(db_select("select * from reservation where linje_id = $linje_id", __FILE__ . " linje " . __LINE__)))
		batch($linje_id);  #Hvis der ikke manuelt er reserveret varer tages automatisk fra den ldste indkbsordre
...
function lagerstatus($vare_id, $variant_id, $lager, $antal)
{
	...
	if (!$lager) {
		if ($row = db_fetch_array(db_select("select afd from ansatte where navn = '$ref'", __FILE__ . " linje " . __LINE__))) {
			if ($row = db_fetch_array(db_select("select kodenr from grupper where box1='$row[afd]' and art='LG'", __FILE__ . " linje " . __LINE__))) {
				$lager = $row['kodenr'];
			}
		}
	}
...
{
	global $brugernavn;
	global $regnaar, $vis_saet; #20170622
	...
	while ($r = db_fetch_array($q)) {
		$y = $x - 1;
		if ($x && $r['varenr'] == $varenr[$y] && $r['beskrivelse'] == 'rabat')
			$dontInsert = 1;
		else {
			$posnr[$x] = $r['posnr'] * 1;
			$pris[$x] = $r['pris'] * 1;
...
function batch($linje_id)
{
	...
	if (($antal >= 0) && ($art != "DK")) {
		...
		while ($row = db_fetch_array($query)) {
			...
			while ($r2 = db_fetch_array($q2)) {
				if ($r2['linje_id'] != $linje_id) {
					$reserveret[$x] = $reserveret[$x] + $r2['antal'];
				} else {
					$valg[$x] = $r2['antal'];
					$valgt .= $r2['antal'];
				}
	...
	if ($lev_rest == 0) {
		...
		for ($x = 1; $x <= $batch_antal; $x++) {
			$lager = $lager * 1;
			if (($valg[$x] > 0) && (!$res_linje_id[$x])) {
				db_modify("insert into reservation (linje_id, vare_id, batch_kob_id, antal, lager) values ($linje_id, $vare_id, $batch_kob_id[$x], $valg[$x], $lager)", __FILE__ . " linje " . __LINE__);
			} elseif (($valg[$x] > 0) && ($res_linje_id[$x])) {
				db_modify("insert into reservation (linje_id, vare_id, batch_salg_id, antal, lager) values ($res_linje_id[$x], $vare_id, $temp, $valg[$x], $lager)", __FILE__ . " linje " . __LINE__);
			}
		}
	}
} # endfinc batch
...
#	fwrite($fp,$linje."\n");
	...
	if ($art == 'PO') {
		...
		$r = db_fetch_array(db_select($qtxt, __FILE__ . " linje " . __LINE__));
		$centDiffAcc = (int) $r['box2'];
		$centDiffVat = 0;
		if ($centDiffAcc) {
			$qtxt = "select id, moms from kontoplan where kontonr='$centDiffAcc'";
			if ($r = db_fetch_array(db_select($qtxt, __FILE__ . " linje " . __LINE__))) {
				if ($r['moms']) {
					$a = substr($r['moms'], 0, 1) . 'M';
					$b = (int) substr($r['moms'], 1);
					$qtxt = "select box2 from grupper where art = '$a' and kodenr = '$b'";
					if ($r = db_fetch_array(db_select($qtxt, __FILE__ . " linje " . __LINE__)))
						$centDiffVat = (float) $r['box2'];
					else
						$centDiffVat = 0;
				}
			}
		...
		if (is_numeric($row['felt_2']) && is_numeric($row['felt_4']) && is_numeric($row['felt_5'])) { #20191001
			$qtxt = "insert into pos_betalinger(ordre_id,betalingstype,amount,valuta,valutakurs) values ('$id','$row[felt_1]','$row[felt_2]','$baseCurrency','100')";
			db_modify($qtxt, __FILE__ . " linje " . __LINE__);
	...
	/*
		   if ($momssats) {
			   $qtxt = "select * from ordrelinjer where ordre_id='$id'and vare_id > 0";
			   $q=db_select($qtxt,__FILE__ . " linje " . __LINE__);
			   while ($r = db_fetch_array($q)) {
				   if (!$r['momsfri'] && !$r['momssats']) {
					   echo __line__." ".$r['id']."<br>";
					   $qtxt = "select gruppe,momsfri from varer where id = '$r[vare_id]'";
					   $r2=db_fetch_array(db_select("$qtxt",__FILE__ . " linje " . __LINE__));
					   if (!$r2['momsfri']) {
						   $qtxt = "update ordrelinjer set momssats = '$momssats', momsfri = '' where id = '$r[id]'";
						   db_modify($qtxt,__FILE__ . " linje " . __LINE__);
					   }
				   }
			   }
...
#	$saet=array();
	...
	if ($saetsum) {
		...
		$diff = $tmp - pos_afrund($tmp, $centDiffAcc, $valutakurs);
	}
	...
	/* echo __line__."$baseCurrency<br>"; */

	if ($valuta && $valuta != $baseCurrency) {
		$qtxt = "select valuta.kurs as kurs, grupper.box3 as difkto from ";
		$qtxt .= "valuta, grupper where grupper.art='VK' and grupper.box1='$valuta' and ";
		$qtxt .= "valuta.gruppe=grupper.kodenr and valuta.valdate <= '$fakturadate' ";
		$qtxt .= "order by valuta.valdate desc";
		/* echo __line__."$qtxt<br>";
			  echo count($xx); */
		if ($r = db_fetch_array(db_select($qtxt, __FILE__ . " linje " . __LINE__))) {
			$valutakurs = (float) $r['kurs'];
			$currDiff = (int) $r['difkto'];
...
#	}
	...
	if ($valuta && $baseCurrency && $valuta != $baseCurrency) {
	...
	if (!$fejl) {
		if ($art != "PO") {
			...
			$diff = 0;
			if ($valuta != $baseCurrency && $betaling == 'Kontant' && !$betaling2) {
				$tmp1 = afrund($sum + $moms, 2);
				$tmp2 = pos_afrund($sum + $moms, $currDiff, $valutakurs);
			...
			if ($konto_id == 0 && $a != $b) {
				$tmp2 = round($retur, 2);
				$tmp1 = pos_afrund($retur, $centDiffAcc, $valutakurs);
			...
			if ($centDiffAcc && $diff) {
				if ($centDiffVat)
					$diffVat = $diff * $centDiffVat / 100;
				else
					$diffVat = 0;
				$nettodiff = $diff - $diffVat;
			}
			if ($nettodiff) {
				$qtxt = "insert into ordrelinjer ";
				$qtxt .= "(posnr,antal,pris,rabat,procent,ordre_id,bogf_konto,momssats,beskrivelse,projekt) ";
				$qtxt .= "values ('0','1', '$nettodiff', 0,100, '$id', '$centDiffAcc', '$centDiffVat','Afrunding','0')";
				db_modify($qtxt, __FILE__ . " linje " . __LINE__);
		...
		if(isset($_SESSION["payment_id"]) && $_SESSION["payment_id"] != '') {
			...
			$query = db_select("SELECT betalings_id FROM ordrer WHERE id = '$id'", __FILE__ . " linje " . __LINE__);
		...
		if (strstr($r['box5'], ';'))
			list($straksbogfor, $tmp) = explode(';', $r['box5']); # 20170404 
		else
			$straksbogfor = $r['box5'];
		if ($art == 'PO') {
		...
		$svar = momsupdat($id);
		if ($art == 'PO' && !$konto_id) {
			$r = db_fetch_array(db_select("select box9 from grupper where art='POS' and kodenr='1' and fiscal_year = '$regnaar'", __FILE__ . " linje " . __LINE__));
			$straksbogfor = $r['box9'];
		}
...
function momsupdat($id)
{
	...
	while ($r = db_fetch_array($q)) {
		if ($r['rabatart'] == 'amount')
			$linjesum = ($r['pris'] - $r['rabat']) * $r['antal'];
		else
			$linjesum = ($r['pris'] - ($r['pris'] / 100 * $r['rabat'])) * $r['antal'];
		...
		if ($r['vare_id'] && $r['momsfri'] != 'on' && !$r['omvbet']) {
			...
			if ($varemomssats != $momssats)
				$antal_diff_moms++;
			if ($r['rabatart'] == 'amount')
				$linjemoms = ($r['pris'] - $r['rabat']) * $r['antal'] / 100 * $varemomssats;
			else
				$linjemoms = ($r['pris'] - ($r['pris'] / 100 * $r['rabat'])) * $r['antal'] / 100 * $varemomssats;
			if ($r['procent'] || $r['procent'] == '0')
...
function batch_salg($id)
{
	...
	for ($x = 0; $x < count($batch_id); $x++) {
		...
		$row2 = db_fetch_array($query2);
		$gruppe = $row2['gruppe'];
		$query2 = db_select("select * from grupper where art='VG' and kodenr='$gruppe' and fiscal_year = '$regnaar'", __FILE__ . " linje " . __LINE__);
		$row2 = db_fetch_array($query2);
		$box1 = trim($row2['box1']);
		$box2 = trim($row2['box2']);
		$box3 = trim($row2['box3']);
		...
		if ($box9 == 'on') { # box 9 betyder at der anvendes batch styring
			if ($antal[$x] > 0) {
				...
				if ($row['fakturadate'])
					$kostpris = $row['pris'] * 1; #Hvis fakturadatoen ikker er sat, er købsordren ikke bogført og kostprisen fra ordrelinjer anvendes.
				else
	...
	if ($art == 'PO') { #201810112
		...
		while ($r = db_fetch_array($q)) {
			...
			$r2 = db_fetch_array($q2);
			if ($r2['id'])
				db_modify("update batch_salg set antal='0' where linje_id='$r2[id]'", __FILE__ . " linje " . __LINE__);
		}
	}
} # endfunc batch_salg
...
function bogfor_indbetaling($id, $webservice) {
	...
	if ($r = db_fetch_array($q)) {
		...
		$udlign_id = $r['id'];
		if ($openpost_id && $udlign_id)
			db_modify("update openpost set udlignet = '1' where id='$udlign_id' or id='$openpost_id'", __FILE__ . " linje " . __LINE__);
		# <- 20161001
		$r = db_fetch_array(db_select("select gruppe from adresser where id='$konto_id'", __FILE__ . " linje " . __LINE__));
		$qtxt = "select beskrivelse, box2 from grupper where art = 'DG' and kodenr='" . (int) $r['gruppe'] . "' and fiscal_year = '$regnaar'";
		$r = db_fetch_array(db_select($qtxt, __FILE__ . " linje " . __LINE__));
		...
		if ($art == 'PO' && $konto_id && is_numeric($konto_id)) { #20190116
			...
			$q = db_select($qtxt, __FILE__ . " linje " . __LINE__);
			if ($r = db_fetch_array($q)) {
				$qtxt = "select box2 from grupper where art='KG' and kodenr='$r[gruppe]' and fiscal_year = '$regnaar'";
				$r = db_fetch_array(db_select($qtxt, __FILE__ . " linje " . __LINE__));
				$kontonr = $r['box2'];
		...
		if ($betaling == 'Kontant') {
			...
			for ($x = 0; $x < count($korttyper); $x++) {
				if (strtolower($betaling) == strtolower($korttyper[$x])) { #20240305
...
function bogfor_nu($id, $kilde) {
	...
	if ($webservice)
		...
		$qtxt = "select kontonr,moms from kontoplan where regnskabsaar='$regnaar' and (kontotype='D' or kontotype='S')";
	$q = db_select($qtxt, __FILE__ . " linje " . __LINE__);
	while ($r = db_fetch_array($q)) {
	...
	$x = 0;
	$qtxt = "select distinct(box1) from grupper where art = 'SM' and box2 != '0.00' and fiscal_year = '$regnaar' order by box1"; #20190428
	$q = db_select($qtxt, __FILE__ . " linje " . __LINE__);
	while ($r = db_fetch_array($q)) {
		$vatAccount[$x] = $r['box1'];
		$vatAmount[$x] = 0;
	...
	while ($r = db_fetch_array($q)) {
		...
		while ($r2 = db_fetch_array($q2)) {
			if ($r2['momssats'] && !$r2['momsfri']) {
				if (!$r2['vat_account'])
					$r2['vat_account'] = $vatAccount[0]; # kan fjernes fra ver 3.7.6 
	...
	if ($konto_id && $kontonr) {
		...
		$r = db_fetch_array(db_select("select box1 from grupper where art='DG' and kodenr='$debitorgruppe' and fiscal_year = '$regnaar'", __FILE__ . " linje " . __LINE__));
	...
	if (!is_numeric($id) || $betalingsbet == 'Kontant' || $betalingsbet == 'Kreditkort' || ($betalingsbet == 'Forud' && $art == 'PO' && $felt_1 != 'konto')) { #20130820+20150306
		...
		$openpost = NULL; #20130917 + #20140115 
	else
		$openpost = 1;
	if (substr($art, 1, 1) == 'K' && $openpost && $kred_ord_id) {
		...
		elseif ($webservice && $betalings_id)
			$beskrivelse = "Web: " . $kundeordnr . ", Bet.ID: " . $betalings_id; #20181009
	} elseif ($openpost)
		$beskrivelse = "Faktura - " . $fakturanr;
	elseif ($webservice)
		$beskrivelse = "Web: " . $kundeordnr . ", Fakt: " . $fakturanr; #20180914
	...
	if ($kontonr && $konto_id && $openpost && is_integer($id)) { #20161211
		...
		$openpost_id = $r['id'];
		$r = db_fetch_array(db_select("select gruppe from adresser where id='$konto_id'", __FILE__ . " linje " . __LINE__));
		$r = db_fetch_array(db_select("select beskrivelse, box2 from grupper where art = 'DG' and kodenr='$r[gruppe]' and fiscal_year = '$regnaar'", __FILE__ . " linje " . __LINE__));
		$kontonr = $r['box2']; # Kontonr aendres fra at vaere leverandoerkontonr til finanskontonr
	...
	if ($art == 'PO') { #saa er det en kontantordre (POS) 
		$retur = $sum;
		$tmparray = array();
		$qtxt = "select box6 from grupper where art = 'POS' and kodenr = '2' and fiscal_year = '$regnaar'";
		$r = db_fetch_array(db_select($qtxt, __FILE__ . " linje " . __LINE__));
		$div_kort_kto = trim($r['box6']);
		$qtxt = "select * from grupper where art = 'POS' and kodenr = '1' and fiscal_year = '$regnaar'";
		$r = db_fetch_array(db_select($qtxt, __FILE__ . " linje " . __LINE__));
		...
		if ($div_kort_kto) { #20140129
			for ($b = 0; $b < count($betaling); $b++) {
				if (
					!in_array(strtolower($betaling[$b]), $l_cardNames)
			...
			for ($b = 0; $b < count($betaling); $b++) {
				...
				if (strtolower($betaling[$b]) == strtolower($korttyper[$x])) { #20170816
					...
					if ($r = db_fetch_array(db_select($qtxt, __FILE__ . " linje " . __LINE__))) {
						...
						$qtxt = "select box1,box2 from grupper where art = 'SM' ";
						$qtxt .= "and kode='S' and kodenr = '$kodenr' and fiscal_year = '$regnaar'";
						if ($r = db_fetch_array(db_select($qtxt, __FILE__ . " linje " . __LINE__))) {
	...
	if ($valutakurs)
		$maxdif = 2; #Der tillades 2 oeres afrundingsdiff
	$p = 0;
	$projektliste = '';
	if (is_numeric($id))
		$tmp = "ordre_id = '" . $id . "'";
				...
				if (!in_array($r['bogf_konto'], $bogf_konto)) {
					...
					if ($r['rabatart'] == 'amount') {
						...
						if ($art == 'PO')
							...
							$pris[$y] = afrund($pris[$y], 3); #Afrunding tilfoejet 20090126 grundet diff i ordre 98 i saldi_104 -- 20110207 ændret til 2 decimaler ordre_id 1325 saldi_329
					}
					...
					for ($a = 1; $a <= $y; $a++) {
						if ($bogf_konto[$a] == $r['bogf_konto'] && $vat_account[$a] == $r['vat_account']) {
							...
							if ($art == 'PO')
								...
								$pris[$a] = afrund($pris[$a], 3); #Afrunding tilfoejet 20090126 grundet diff i ordre 98 i saldi_104 -- 20110207 ændret til 2 decimaler ordre_id 1325 saldi_329
							if ($r['momssats'] && !$r['momsfri'])
								$linjemoms[$a] += afrund($linjesum / 100 * $r['momssats'], 3);
						}
					}
				}
			}
...
*/
	...
	if ($brugernavn == 'saldi')
		echo __LINE__ . " $d_kontrol != $k_kontrol<br>";
	...
	if ($diff = afrund($d_kontrol - $k_kontrol, 2)) {
		...
		$kredit = afrund($kredit, 2);
		/*if ($brugernavn == 'saldi') {
				  echo __LINE__ . " Valuta $valuta - Diff $diff - D $debet K $kredit<br>";
				  #	exit;
			  }
			  if ($art == 'PO') { #20140628
				  $r = db_fetch_array(db_select("select box9 from grupper where art = 'POS' and kodenr = '2' and fiscal_year = '$regnaar'", __FILE__ . " linje " . __LINE__)); #20170826 +4 linjer
		...
		if ($art == 'PO') { #20140628
		...
		} elseif ($valuta != $baseCurrency && abs($diff) <= $maxdif) { #Der maa max vaere en afvigelse paa 1 oere pr ordrelinje m fremmed valuta;
			...
			$svar = "Der er konstateret en uoverensstemmelse i posteringssummen, ID $ordre_id ordre $ordrenr, d=$d_kontrol, k=$k_kontrol kontakt Saldi p&aring; telefon 4690 2208";
...
function stamkunder($art, $sort, $fokus, $id, $kontonr, $firmanavn, $addr1, $addr2, $postnr, $bynavn, $kontakt, $sum)
{
	...
	$kolonner = 12;
	$start = 0;
	$firmanavn = $konto_id = array();
	if (isset($_GET['stamkunder']))
		$start = $_GET['stamkunder'];
	...
	for ($x = $start; $x < count($konto_id); $x++) {
		...
		if ($y >= $kolonner) {
			...
			if ($linje == 0) {
				for ($i = $y + 1; $i < $kolonner; $i++)
					print "<td style=\"width:110px\"></td>";
				print "<td>$kontantknap</td>";
			}
			if ($linje == $linjer - 1) {
				for ($i = $y + 1; $i < $kolonner; $i++)
...
#				}
		...
		$style = $stil . "$bgcolor; color: #fff\"";
		#	$firmanavn[$x]=str_replace("<br>"," \n ",$firmanavn[$x]);
		while (strlen($firmanavn[$x]) < 12)
			$firmanavn[$x] = " " . $firmanavn[$x] . " ";
...
function ansatopslag($sort, $fokus, $id)
{
	...
	print "<tbody><tr>";
	print "<td><b><a href=ordre.php?sort=navn&funktion=ansatOpslag&fokus=$fokus&id=$id>$txt646</b></td>";
	...
	print "<td><b><a href=ordre.php?sort=email&funktion=ansatOpslag&fokus=$fokus&id=$id>$txt402</b></td>";
	print " </tr>\n";
...
{ #20140426
	...
	if (isset($_SESSION['varenr_ny']) && $_SESSION['varenr_ny']) {
		if ($varenr == $barcodeNew)
			$varenr = $_SESSION['varenr_ny'];
	...
	if ($procent == '')
		$procent = 100;
	if (!is_numeric($fast_db))
		$fast_db = 0;
	if (!is_numeric($rabat_ny))
		$rabat_ny = 0;
	...
	if ($qtxt && $r = db_fetch_array(db_select("$qtxt", __FILE__ . " linje " . __LINE__))) {
		...
		$productPrice = $r['salgspris'];
		$productCost = $r['kostpris'];
		...
		if ($bogfkto && !$momsfri) {
			$qtxt = "select var_value from settings where var_name = 'maxDepVatRate_" . $afd . "'"; #20241228
			if ($r = db_fetch_array(db_select($qtxt, __FILE__ . " linje " . __LINE__))) {
				$qtxt = "select box1,box2 from grupper where fiscal_year = '$regnaar' and kodenr = '$afd' and art = 'SM'";
				if ($r = db_fetch_array(db_select($qtxt, __FILE__ . " linje " . __LINE__))) {
					if ($r['box1'])
						$vatAccount = (int) $r['box1'];
					if ($r['box2'])
						$varemomssats = (float) $r['box2'];
				}
			} else {
		...
		if (
			($special_from_date < $dd || ($special_from_date == $dd && $special_from_time <= $tt)) &&
			($special_to_date > $dd || ($special_to_date == $dd && $special_to_time >= $tt))
		)
			$SpecialPeriod = 1;
		if ($SpecialPeriod && $special_price && $specialType == 'percent') {
			if ($rabat_ny == 0)
				$rabat_ny = $special_price;
			$special_price = 0;
		}
		if (!$pris) {
			$ugedag = date('N');
			...
			if ($r2 = db_fetch_array(db_select($qtxt, __FILE__ . " linje " . __LINE__))) {
				...
				$pris = $special_price;
				$kostpris = $campaignCost * 1;
			} else {
				if ($pris != '0')
					$pris = $productPrice; #20140124
			...
			$ms = $varemomssats;
			if ($momssats < $varemomssats)
				$varemomssats = $momssats;
			if ($incl_moms && $varemomssats)
				$pris = $pris - ($pris * $varemomssats / (100 + $varemomssats)); # 20190111 fjernet: $art=='PO' &&
			else
				$pris *= 1; #20140124
			$kostpris = $productCost * 1;
		}
	...
	if (!is_numeric($rabatgruppe))
		$rabatgruppe = 0;
	if (!is_numeric($varerabatgruppe))
		$varerabatgruppe = 0;
	$vare_id *= 1;
	$m_rabat = 0;
	$rabat_ny *= 1;
	...
	if (!$folger && !$saet && ((!$kopi && $linje_id && $art == 'DO') || $art == 'PO') && $r = db_fetch_array(db_select($qtxt, __FILE__ . " linje " . __LINE__))) { #20200109
		...
		if (($art != 'PO' || $antal) && $antaldiff && $r['id']) {
			if (abs($antal) < 100000000000) {
				...
				print "<BODY onLoad=\"javascript:alert('Antal (" . dkdecimal($antal) . ") er for stort, reducer antal')\">";
		...
		if ($kopi || $rabat_ny)
			...
			if (!is_numeric($debitorrabatgruppe))
				$debitorrabatgruppe = 0;
			if (!is_numeric($varerabatgruppe))
				$varerabatgruppe = 0;
			if (!isset($rabat))
				$rabat = 0;
...
function find_pris($varenr)
{
	...
	if (!$pris && $r = db_fetch_array(db_select($qtxt, __FILE__ . " linje " . __LINE__))) {
		...
		$special_from_time = $r['special_from_time'];
		$special_to_time = $r['special_to_time'];
		if ($special_from_date < $dd && $special_from_time != '00:00:00')
			$special_from_time = '00:00:00'; #20160909
		if ($special_to_date > $dd && $special_to_time == '00:00:00')
			$special_to_time = '23:59:59'; #20160909
		$ugedag = date('N');
		$uxtid = date("U");
		$tidspkt = date("H:i:s");
		...
		if ($r2 = db_fetch_array(db_select($qtxt, __FILE__ . " linje " . __LINE__)) && $pris = $r2['salgspris']) {
			...
			$specialType == 'price' && $special_price && $special_from_date <= $dd &&
		...
		$qtxt = "select box4,box7 from grupper where art = 'VG' and kodenr = '$vg' and fiscal_year = '$regnaar'";
		$r = db_fetch_array(db_select($qtxt, __FILE__ . " linje " . __LINE__));
		$momsfri = $r['box7'];
		if (!$momsfri) {
			$kontonr = $r['box4'] * 1;
...
{ #Returnerer kostpris i baseCurrency
	...
	while ($r = db_fetch_array($q)) {
		...
		if ($batch_kob_id) {
			...
			if ($r2 = db_fetch_array(db_select($qtxt, __FILE__ . " linje " . __LINE__)))
				$tmp = $r2['pris'] . chr(9) . '0' . chr(9) . $antal . chr(9) . '0' . chr(9) . 'KO';
			else {
		...
		if ($koid[$x]) {
			...
			if ($r3['valutakurs'])
				$valutakurs = $r3['valutakurs']; #20140117
			else
				$valutakurs = 100;
			if ($r3['art'] == 'KO') {
				$qtxt = "select pris from ordrelinjer where id = '$r2[linje_id]'";
				$r2 = db_fetch_array(db_select($qtxt, __FILE__ . " linje " . __LINE__));
...
#				$kostpris[$x]=dkdecimal($r2['pris']*$valutakurs/100);
		...
		if (!$koid[$x]) {
			$qtxt = "select varer.kostpris,grupper.box8 from varer,grupper "; #20240418
			$qtxt .= "where varer.id = '$vare_id' and grupper.art = 'VG' and grupper.kodenr = varer.gruppe";
			$r2 = db_fetch_array(db_select($qtxt, __FILE__ . " linje " . __LINE__));
			$lagerfort = $r2['box8'];
			#20150119 ->
...
function find_momssats($id, $kasse)
{
	...
	elseif ($kasse) {
		$tmp = array();
		$r = db_fetch_array(db_select("select box7 from grupper where art = 'POS' and kodenr = '1' and fiscal_year = '$regnaar'", __FILE__ . " linje " . __LINE__));
		$tmp = explode(chr(9), $r['box7']);
		$momsgrp = $tmp[$kasse - 1]; # Kasseraekken starter med 1 og momsraekken med 0;
		$r = db_fetch_array(db_select("select box2 as momssats from grupper where art = 'SM' and kodenr = '$momsgrp' and fiscal_year = '$regnaar'", __FILE__ . " linje " . __LINE__));
	}
	$momssats = $r['momssats'] * 1;
	return ($momssats);
}
...
function tekstopslag($sort, $id)
{
	...
	sidehoved($id, "ordre.php", "", $fokus, findtekst(1092, $sprog_id) . " $id - Vareopslag");
	...
	while ($r = db_fetch_array($q)) {
		...
		if ($sag_id)
			print "<span style=\"width:16px; display:block;\"><span class=\"xmark\"></span></a></td>\n"; #20170601 indsat 'if ($sag_id)'
		else
			print "<!--Tekst 493--><img src=\"../img/x_mark.png\"></a></td>\n";
		print "</tr>\n";
	}
	print "</tbody>\n";
...
function sidehoved($id, $returside, $kort, $fokus, $tekst)
{
	...
	if ($kort == "../lager/varekort.php" || $kort == "../debitor/debitorkort.php")
		$ny_id = $id;
	else
		$ny_id = 0;
...
function opret_ordre($sag_id, $konto_id)
{
	...
	if ($row = db_fetch_array($query)) {
		...
		if (!$sag_id)
			$kontakt = db_escape_string($row['kontakt']); # 20140821-1
	...
	if ($gruppe) {
		$r = db_fetch_array(db_select("select box1,box3,box4,box6,box8 from grupper where art='DG' and kodenr='$gruppe' and fiscal_year = '$regnaar'", __FILE__ . " linje " . __LINE__));
		$tmp = substr($r['box1'], 1, 1) * 1;
		#		$rabatsats=$r['box6']*1;
		$formularsprog = $r['box4'];
		$valuta = $r['box3'];
		$b2b = $r['box8'];
		$r = db_fetch_array(db_select("select box2 from grupper where art='SM' and kodenr='$tmp' and fiscal_year = '$regnaar'", __FILE__ . " linje " . __LINE__));
		$momssats = $r['box2'] * 1;
	} elseif ($konto_id) {
...
function lagervaerdi($ordre_id)
{
	...
	$vkob = array();
	$lagre = array();
	$q = db_select("select * from grupper where art = 'VG' and fiscal_year = '$regnaar' order by box2", __FILE__ . " linje " . __LINE__);
	while ($r = db_fetch_array($q)) {
		if ($r['box2']) {
	...
	for ($x = 0; $x < count($lagre); $x++) {
		$q = db_select("select kodenr from grupper where art = 'VG' and box2 = '$lagre[$x]' and fiscal_year = '$regnaar'", __FILE__ . " linje " . __LINE__);
		while ($r = db_fetch_array($q)) {
...
function slet_ordre($ordre_id)
{
	...
	# <- 20171031
	$x = 0;
	$q = db_select("select kodenr from grupper where art = 'VG' and box8='on' and fiscal_year = '$regnaar' order by kodenr", __FILE__ . " linje " . __LINE__);
	while ($r = db_fetch_array($q)) {
		$gruppe[$x] = $r['kodenr'];
		$x++;
	}
...
if (!function_exists('find_nextfakt')) {
	function find_nextfakt($fakturadate, $nextfakt)
	{
		...
		$nextfaktultimo = 0;
		$tmp = $faktdag + 1;
		if (!checkdate($faktmd, $tmp, $faktaar))
			$faktultimo = 1; # hvis dagen efter fakturadag ikke findes fakureres ultimo"
		$tmp = $nextfaktdag + 1;
		if (!checkdate($nextfaktmd, $tmp, $nextfaktaar))
			$nextfaktultimo = 1;
		$faktmd_len = 31;
		while (!checkdate($faktmd, $faktmd_len, $faktaar))
			$faktmd_len--; #finder antal dage i fakturamaaneden
			...
			if ($nextfaktdag > $faktmd_len) {
				while (!checkdate($nextfaktmd, $nextfaktdag, $nextfaktaar)) {
					...
					if ($nextfaktdag > $faktmd_len) {
						...
						while (!checkdate($nextfaktmd, $faktmd_len, $nextfaktaar))
							$faktmd_len--;
						$nextfaktdag -= $faktmd_len;
						if ($nextfaktdag < 1)
							$nextfaktdag = 1; #20171101
					}
				}
			} else
				while (!checkdate($nextfaktmd, $nextfaktdag, $nextfaktaar))
...
if (!function_exists('registrer_betaling')) {
	...
	{
		global $db, $db_skriv_id, $regnaar, $sprog_id;
		...
		if ($betalingstype == '!') {
			...
			if ($godkendt) {
				...
				$r = db_fetch_array(db_select($qtxt, __FILE__ . " linje " . __LINE__));
				$korttyper = explode(chr(9), $r['box5']);
				$qtxt = "select box5 from grupper where art = 'POS' and kodenr = '2' and fiscal_year = '$regnaar'";
				$r = db_fetch_array(db_select($qtxt, __FILE__ . " linje " . __LINE__));
				...
				for ($x = 0; $x < count($korttyper); $x++) {
					if ($betal_type != $korttyper[$x] && $betalingskort[$x]) {
						print "<tr><td align='center'><a href='pos_ordre.php?id=$ordre_id&modtaget=$amount&korttype=$korttyper[$x]&betaling=$korttyper[$x]&indbetaling=$indbetaling&godkendt=OK' onclick=\"return confirm('$txt')\"><button style=\"height:100px;width:300px;\">$korttyper[$x]</button></a></td></tr>";
					}
				}
