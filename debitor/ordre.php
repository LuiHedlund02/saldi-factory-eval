...
if (!$id && $konto_id && $kontonr && !strstr($b_submit, 'Opslag')) {
	...
	if ($sku != null) {
		...
		$query = db_select("select * from varer where varenr='$sku' or varenr_alias='$sku' or stregkode='$sku' limit 1", __FILE__ . " linje " . __LINE__);
...
if ($b_submit) {
	...
	$formularsprog   = if_isset($_POST, 'Dansk', 'sprog');
			...
			elseif ($vare_id[$x] || isset($_GET['varenr'])) { #20241229
				...
				if ((($tidl_lev[$x] < 0) && ($antal[$x] > $tidl_lev[$x])) || (($tidl_lev[$x] > 0) && ($antal[$x] < $tidl_lev[$x]))) {
					...
					if (($tidl_lev[$x] < $antal[$x]) && ($status > 1)) {
						$alert = findtekst('1837|Du kan ikke fakturere, før alt er leveret', $sprog_id);
						if ($omdan_t_fakt == "on") {
							print "<BODY onLoad=\"javascript:alert('$alert')\">\n";
						}
						$status = 1;
...
function ordreside($id, $regnskab)
{
	...
	if ($id) {
		...
		$formularsprog = if_isset($row, 'Dansk', 'sprog');
			...
			while ($r = db_fetch_array($q)) {
				...
				switch ($var_name) {
					...
					case 'dfm_url':
						...
						break;
					case 'dfm_delrem':
						$dfm_delrem          = $var_value;
						break;
					case 'dfm_gooddes':
						$dfm_gooddes         = $var_value;
						break;
					case 'dfm_sercode':
	...
	if ($status >= 3) {
		...
		if (db_fetch_array(db_select("select distinct sprog from formularer where sprog != 'Dansk'", __FILE__ . " linje " . __LINE__))) {
			print "<td title=\"" . findtekst('1468|Sprog som skal anvendes på dokumenter som tilbud, ordrer, fakturaer med videre.', $sprog_id) . "\"><b>" . findtekst('571|Baggrund', $sprog_id) . "</b></span></td>\n";
		...
		*/
		...
		for ($x = 1; $x <= $linjeantal; $x++) {
			...
			if ($kobs_ordre_id[0] && $art != 'DK' && $ko_ant[$x] >= 1) {
				...
				if ($serienr[$x]) {
					print "<td onClick=\"serienummer($linje_id[$x])\" onMouseOver=\"this.style.cursor = 'pointer'\" align=\"right\" title=\"" . findtekst('1497|Serienummer', $sprog_id) . " \"><img alt=\"" . findtekst('1497|Serienummer', $sprog_id) . "\" src=\"../ikoner/serienr.png\"></td>\n";
				}
		}
		# 20150412
		if ($brugsamletpris) {
		...
		print "</SELECT></td></tr>\n";
		if (db_fetch_array(db_select("select distinct sprog from formularer where sprog != 'Dansk'", __FILE__ . " linje " . __LINE__))) {
			print "<tr><td title=\"" . findtekst('1468|Sprog som skal anvendes på dokumenter som tilbud, ordrer, fakturaer med videre.', $sprog_id) . "\">" . findtekst('571|Baggrund', $sprog_id) . "</span></td>\n";
		...
		*/
		...
		for ($x = 1; $x <= $linjeantal; $x++) {
			...
			if ($saet[$x] && $saetpris) {
				...
				if (!$samlevare[$x] && $saet[$x] && ($saet[$x + 1] != $saet[$x] || $samlevare[$x + 1])) {
					$r = db_fetch_array(db_select("select id,varenr,beskrivelse,lager,vare_id,pris,antal,rabat from ordrelinjer where saet = '$saet[$x]' and ordre_id='$id' and samlevare='on'", __FILE__ . " linje " . __LINE__));
					if ($r) { # Render even if varenr is empty (ordrelinjer now handles this for collections)
						# The hlavovaren's pris holds the rounding diff between sæt total and sum of sub-items.
						# When discount is applied, this diff also gets rabat but is excluded from $sum
						# (because ordrelinjer skips samlevare='on'). Add the discounted diff to $sum here.
						$hlavovaren_rabat = (float)$r['rabat'];
						$hlavovaren_diff = $r['pris'] * (1 - $hlavovaren_rabat / 100);
						$sum += $hlavovaren_diff;
		...
		if ($kontonr) { # && !$disabled
			...
			if ($varenr[0] && isset($_GET['vare_id']) || isset($_GET['varenr'])) { //20150407+20241229
				...
				if (isset($_GET['varenr'])) {
					...
					$r = db_fetch_array(db_select("select * from varer where varenr='$varenr[0]' or varenr_alias='$varenr[0]' or stregkode='$varenr[0]'", __FILE__ . " linje " . __LINE__));
        ...
        size='10'>
			...
			if ($procenttillag) {
				$r = db_fetch_array(db_select("select beskrivelse from varer where varenr = '$procentvare' or varenr_alias = '$procentvare' or stregkode = '$procentvare'", __FILE__ . " linje " . __LINE__));
		...
		// TODO: Change button border radius here
		if ($status < 3) {
			if ($levdiff) $status = 1;
			...
			print "<td align=\"center\" width=$width><input type=\"submit\" class=\"button green medium\" id=\"submit\" style=\"width:75px; border-radius: 4px;\" accesskey=\"g\" value=\"$txt\" name = 'save' onclick=\"javascript:docChange = false;\"></td>\n";

			if ($art != 'OT') { # Fjerner knappen opslag hvis art er = OT (original tilbud) #20140716
				$txt = findtekst('644|Opslag', $sprog_id);
				print "<td align=\"center\" width=$width><input type=\"submit\" class=\"button blue medium\" style=\"width:75px; border-radius: 4px;\" accesskey=\"o\" value=\"$txt\" name ='lookUp' "; #
			...
			if (count($leveret) == 0 && !$betalt && $art != 'OT' && $id) {
				print "<td align=\"center\"><input type=\"submit\" class=\"button rosy medium\" style=\"width:75px; border-radius: 4px;\" value=\"" . findtekst('1099|Slet', $sprog_id) . "\" name=\"delete\" onclick=\"return confirm('$tekst')\" title=\"$tekst2\"></td>\n";
			}
...
function ordrelinjer($x, $sum, $dbsum, $blandet_moms, $moms, $antal_ialt, $leveres_ialt, $tidl_lev_ialt, $levdiff, $masterprojekt, $linje_id, $kred_linje_id, $posnr, $varenr, $beskrivelse, $enhed, $lager, $pris, $rabat, $rabatart, $procent, $antal, $leveres, $leveret, $vare_id, $momsfri, $rabatgruppe, $m_rabat, $varemomssats, $serienr, $samlevare, $folgevare, $projekt, $kdo, $kobs_ordre_pris, $ko_ant, $kostpris, $dkb, $dg, $dk_db, $dk_dg, $readonly, $omvbet, $saet, $saetnr, $grossWeight, $netWeight, $itemLength, $itemWidth, $itemHeight, $volume, $linje)
{
	...
	print "<td class='drag-handle' style='cursor:grab; text-align:center; vertical-align:middle; width:30px; background:#f5f5f5;'>⋮⋮</td>";
...
{
	$query = db_select("select id from varer where varenr = '$varenr' or varenr_alias = '$varenr' or stregkode = '$varenr'", __FILE__ . " linje " . __LINE__);
	if ($row = db_fetch_array($query)) {
		return $row[id];
	}
}
